...
async def _safety_conflicts(db: AsyncSession, patient_id: str, drug_ids: list[str]) -> list[str]:
    ...
    contraindications. Substring matching on free text â€” deliberately
    ...
    for drug in drugs:
        ...
        for allergen in allergies:
            if allergen in drug_names:
                conflicts.append(f"ALLERGY: patient is allergic to '{allergen}' â€” {drug.name}")
    ...
    if conflicts:
        if not body.override_allergy_block:
            raise HTTPException(
                ...
                detail={"message": "Drug safety conflict â€” prescription blocked", "conflicts": conflicts,
...
async def dispense(
    ...
    # Allergy/contraindication gate at the dispensing counter (audit C6) â€”
    ...
    if conflicts:
        if not body.override_allergy_block:
            raise HTTPException(
                status_code=status.HTTP_409_CONFLICT,
                detail={"message": "Drug safety conflict â€” dispense blocked", "conflicts": conflicts,
                        "hint": "Set override_allergy_block=true to override; the override is audit-logged."},
            )
        await log_action(
    ...
    for dispense_item in body.items:
        ...
        # Cap at the prescribed amount â€” dispensing must never exceed what the
        ...
        # Deduct from stock (FEFO â€” first to expire first)
