...
export default function Clinical() {
  ...
  const [visitEditForm, setVisitEditForm] = useState({ chief_complaint: "" });
  ...
  const openEditVisit = (v, e) => {
    e.stopPropagation();
    setVisitEditForm({ chief_complaint: v.chief_complaint || v.notes || "" });
    setEditingVisit(v);
  };
  ...
  return (
    <div className="page-container space-y-6">
        ...
        <div className="bg-white rounded-lg border border-border">
          ...
          <div className="divide-y divide-border max-h-[600px] overflow-y-auto">
            {visits.map(v => {
              ...
              return (
                <div key={v.id} className={`relative group border-l-2 ${selectedVisit?.id === v.id ? "bg-primary/5 border-l-primary" : priority === "emergency" ? "border-l-triage-emergency" : priority === "urgent" ? "border-l-triage-urgent" : "border-l-transparent"}`}>
                  <button onClick={() => selectVisit(v)} className="w-full text-left p-3 hover:bg-muted/40 transition-colors">
                  ...
                  <button onClick={(e) => openEditVisit(v, e)} className="absolute top-2 right-2 p-1 rounded opacity-0 group-hover:opacity-100 hover:bg-muted text-muted-foreground transition-opacity" title="Edit visit">
      ...
      {editingVisit && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          <div className="absolute inset-0 bg-black/40" onClick={() => setEditingVisit(null)} />
          <div className="relative bg-card rounded-xl p-6 shadow-2xl w-full max-w-sm mx-4">
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-heading text-lg font-semibold">Edit Visit</h3>
              <button onClick={() => setEditingVisit(null)} className="p-1.5 rounded-lg hover:bg-muted"><X className="w-4 h-4" /></button>
            </div>
            <p className="text-xs text-muted-foreground mb-4">{getPatientName(editingVisit.patient_id)} · {editingVisit.visit_type}</p>
            <div>
              <label className="block text-xs font-medium text-muted-foreground mb-1">Chief Complaint</label>
              <textarea rows={4} className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm resize-none focus:outline-none focus:ring-2 focus:ring-ring" value={visitEditForm.chief_complaint} onChange={e => setVisitEditForm({ ...visitEditForm, chief_complaint: e.target.value })} placeholder="Reason for visit..." />
            </div>
            <div className="flex gap-3 mt-4">
              <button onClick={handleSaveVisitEdit} className="px-4 py-2 bg-primary text-primary-foreground rounded-lg text-sm font-medium flex items-center gap-1"><Save className="w-3.5 h-3.5" /> Save</button>
              <button onClick={() => setEditingVisit(null)} className="px-4 py-2 border border-border rounded-lg text-sm">Cancel</button>
            </div>
          </div>
        </div>
      )}
