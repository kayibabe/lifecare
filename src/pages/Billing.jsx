...
export default function Billing() {
  ...
  const createInvoice = async (e) => {
    ...
    try {
      ...
      toast({ title: "Failed to create invoice", description: err.response?.data?.detail || err.message, variant: "destructive" });
    } finally {
  ...
  const openEditInvoice = (inv) => {
    setInvoiceEditForm({ payment_mode: inv.payment_mode || inv.payment_type || "cash", discount: inv.discount ?? "", notes: inv.notes || "" });
  ...
  return (
    <div className="page-container">
          ...
          <form onSubmit={createInvoice} className="space-y-4">
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex gap-3">
                <button type="submit" disabled={creatingInvoice} className="px-6 py-2.5 bg-primary text-primary-foreground rounded-lg text-sm font-medium disabled:opacity-50"><Save className="w-3 h-3 inline mr-1" />{creatingInvoice ? "Creating…" : "Create Invoice"}</button>
                        ...
                        <td className="py-2.5 px-3">
                          <div className="flex gap-1 flex-wrap">
                            {inv.status !== "paid" && inv.status !== "cancelled" && (
                              <button onClick={() => openEditInvoice(inv)} className="px-2 py-1 bg-muted text-muted-foreground rounded text-xs font-medium hover:bg-muted/80"><Pencil className="w-3 h-3 inline mr-0.5" /> Edit</button>
                            )}
                            {inv.status !== "paid" && inv.status !== "cancelled" && (
                              <button onClick={() => {
      ...
      {editingInvoice && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          <div className="absolute inset-0 bg-black/40" onClick={() => setEditingInvoice(null)} />
          <div className="relative bg-card rounded-xl p-6 shadow-2xl w-full max-w-sm mx-4">
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-heading text-lg font-semibold">Edit Invoice</h3>
              <button onClick={() => setEditingInvoice(null)} className="p-1.5 rounded-lg hover:bg-muted"><X className="w-4 h-4" /></button>
            </div>
            <p className="text-xs text-muted-foreground mb-4 font-mono">{editingInvoice.invoice_number}</p>
            <div className="space-y-3">
              <div>
                <label className="block text-xs font-medium text-muted-foreground mb-1">Payment Mode</label>
                <select className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm" value={invoiceEditForm.payment_mode} onChange={e => setInvoiceEditForm({ ...invoiceEditForm, payment_mode: e.target.value })}>
                  <option value="cash">Cash</option><option value="insurance">Insurance</option><option value="mixed">Mixed</option>
                </select>
              </div>
              <div>
                <label className="block text-xs font-medium text-muted-foreground mb-1">Discount (MWK)</label>
                <input type="number" min="0" className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm" value={invoiceEditForm.discount} onChange={e => setInvoiceEditForm({ ...invoiceEditForm, discount: e.target.value })} />
              </div>
              <div>
                <label className="block text-xs font-medium text-muted-foreground mb-1">Notes</label>
                <textarea rows={3} className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm resize-none" value={invoiceEditForm.notes} onChange={e => setInvoiceEditForm({ ...invoiceEditForm, notes: e.target.value })} />
              </div>
            </div>
            <div className="flex gap-3 mt-4">
              <button onClick={handleSaveInvoiceEdit} className="px-4 py-2 bg-primary text-primary-foreground rounded-lg text-sm font-medium"><Save className="w-3 h-3 inline mr-1" /> Save</button>
              <button onClick={() => setEditingInvoice(null)} className="px-4 py-2 border border-border rounded-lg text-sm">Cancel</button>
            </div>
          </div>
        </div>
      )}
