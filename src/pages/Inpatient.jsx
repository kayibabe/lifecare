...
export default function Inpatient() {
  ...
  const admitPatient = async (e) => {
    ...
    try {
      ...
      toast({ title: "Failed to admit patient", description: err.response?.data?.detail || err.message, variant: "destructive" });
    } finally {
      setAdmitting(false);
    }
  };
      ...
      {showAdmit && (
        <div className="bg-white rounded-lg border border-border p-6">
          ...
          <form onSubmit={admitPatient} className="space-y-4">
            ...
            <div className="flex gap-3"><button type="submit" disabled={admitting} className="px-6 py-2.5 bg-primary text-primary-foreground rounded-lg text-sm font-medium disabled:opacity-50"><Save className="w-3 h-3 inline mr-1" />{admitting ? "Admitting…" : "Admit"}</button><button type="button" onClick={() => setShowAdmit(false)} className="px-6 py-2.5 border border-border rounded-lg text-sm font-medium hover:bg-muted">Cancel</button></div>
          </form>
        </div>
      )}
