...
export default function Pharmacy() {
  ...
  const addDrug = async (e) => {
    ...
    try {
      ...
      toast({ title: "Failed to save drug", description: err.response?.data?.detail || err.message, variant: "destructive" });
    } finally {
      setSavingDrug(false);
    }
  };
          ...
          {showAddDrug && (
            <form onSubmit={addDrug} className="mb-6 p-4 bg-muted/30 rounded-xl space-y-3">
              ...
              <div className="flex gap-3">
                <button type="submit" disabled={savingDrug} className="px-4 py-2 bg-primary text-primary-foreground rounded-lg text-sm font-medium disabled:opacity-50"><Save className="w-3 h-3 inline mr-1" />{savingDrug ? "Saving…" : "Save Drug"}</button>
