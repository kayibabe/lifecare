...
export default function GeneralClaimForm() {
    ...
    <form className="space-y-6 bg-card border border-border/60 rounded-xl p-6">
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          ...
          <div><label className="block text-xs text-muted-foreground mb-1">Age</label><input className="w-full rounded-lg border border-border px-3 py-2 text-sm bg-muted/50" value={form.age} readOnly type="text" /></div>
      ...
      <button type="submit" className="w-full bg-primary text-primary-foreground rounded-lg py-2.5 font-medium text-sm hover:bg-primary/90 flex items-center justify-center gap-2"><Save className="w-4 h-4" /> Save General Claim</button>
    </form>
  );
}