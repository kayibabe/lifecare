  ...
  const sidebarContent =
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center gap-3 px-4 py-4 border-b border-sidebar-border/60">
        ...
        <div
          className="flex-shrink-0 w-10 h-10 rounded-xl shadow-sm ring-1 ring-black/8"
          style={{
            background: "url('/logo.png') center 0% / 200% auto no-repeat white",
          }}
          aria-label="LifeCare emblem"
        />
        {!collapsed && (
          <div className="overflow-hidden">
            <h1 className="text-[11.5px] font-bold text-sidebar-foreground leading-tight tracking-tight">LifeCare · HIMS</h1>
            <p className="text-[9px] font-semibold text-sidebar-foreground/45 tracking-widest uppercase mt-0.5">Malawi</p>
          </div>
        )}
      </div>
        ...
        {NAV_GROUPS.map((group) => {
          ...
          return (
            <div key={group.label}>
              ...
              {/* Nav items */}
              {!isGroupCollapsed && (
                <div className="space-y-0.5 mt-1 ml-3 pl-3 border-l-2" style={{ borderColor: `${color}35` }}>
      ...
      <div className="border-t border-sidebar-border/60 p-3 space-y-1">
        {/* User card */}
        <div className={`flex items-center gap-2.5 px-2.5 py-2 rounded-lg bg-sidebar-foreground/5 ${collapsed ? "justify-center" : ""}`}>
          <div
          ...
          >
            {(currentUser?.display_name || currentUser?.full_name || "?")[0]?.toUpperCase()}
          </div>
          {!collapsed && (
            <div className="overflow-hidden">
              ...
              <p className="text-[9.5px] text-sidebar-foreground/50 tracking-wide">
            ...
            <div className="relative">
              ...
              >
                ...
                <div className="text-left">
                  ...
                  <p className="text-[10px] text-muted-foreground">{formatRole(currentUser?.role) || "User"}</p>
                </div>
              </button>
              {profileOpen && (
