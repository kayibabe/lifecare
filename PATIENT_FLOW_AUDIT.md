# GPC Patient Flow Audit
**Run date:** 2026-07-04  
**Environment:** Local — FastAPI test client (httpx ASGITransport) against in-memory SQLite  
**Backend:** `D:\WebApps\gpc\backend` — FastAPI 0.x, SQLAlchemy async, Python 3.14  
...
**Steps executed:**
1. `POST /api/v1/patients` → MRN `GPC000001` assigned (auto-sequence, unique)
...
Patient looked up by MRN via `GET /api/v1/patients?q=GPC000002`. Found immediately. Second encounter created without obstruction. Both encounters visible in `GET /api/v1/encounters?patient_id=…`. Full visit history is maintained correctly.
...
- **MRN generation** via PostgreSQL sequence (SQLite fallback in tests): gap-free, prefixed `GPC000001`
